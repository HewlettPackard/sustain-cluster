# WORKING SOLUTION: Simplified training without complex wrappers
# train_ppo_manager_working.py
...
def parse_args():
    parser = argparse.ArgumentParser(description="WORKING: Simplified Parallel Manager Training")
    ...
    parser.add_argument("--tag", type=str, default="working", help="Run tag")
    ...
    parser.add_argument("--llm-max-concurrent", type=int, default=32)
    parser.add_argument("--llm-history-window", type=int, default=10)
    
    return parser.parse_args()

def create_simple_env(rank: int, seed: int, config_paths: dict, llm_config: dict):
    """SIMPLIFIED: Create environment without complex wrappers"""
    import numpy as np
    import pandas as pd
    import datetime
    from envs.sustaincluster_ma_env_llm import LLMIntegratedSustainClusterMAEnv
    ...
    try:
        ...
        # Use pickle-safe reward wrapper
        reward_fn = PickleSafeRewardWrapper(reward_cfg)

        # SIMPLIFIED: Create environment directly without complex wrappers
        env = LLMIntegratedSustainClusterMAEnv(
            ...
            reward_fn=reward_fn,
            logger=None,
            simple_obs_mode=True,  # Always use simple mode for compatibility
            use_meta_tasks=True,
...
class SimpleVectorizedEnv:
    """
    WORKING: Simple custom vectorized environment that avoids SB3 compatibility issues
    """
    def __init__(self, env_fns):
        ...
        print(f"SimpleVectorizedEnv created: {self.num_envs} envs, {self.num_agents} agents each")
...
def make_env(rank: int, seed: int, config_paths: dict, llm_config: dict):
    ...
    def _init():
        return create_simple_env(rank, seed, config_paths, llm_config)
    return _init

async def train():
    """WORKING: Simplified training with custom vectorization"""
    args = parse_args()
    run_name = f"PPO_MGR_WORKING_{args.tag}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    # WORKING: Create environments with simple custom vectorization
    env_fns = [make_env(i, args.seed, config_paths, llm_config) for i in range(args.num_envs)]
    
    logger.info(f"Creating {args.num_envs} environments with custom vectorization")
    env = SimpleVectorizedEnv(env_fns)
    
    # Evaluation environment (single)
    eval_env_fn = make_env(99, args.seed, config_paths, llm_config)
    eval_env = SimpleVectorizedEnv([eval_env_fn])
    ...
    logger.info(f"Environment specs - Obs: {obs_dim}, Action: {action_dim}, Agents: {num_agents}")
    logger.info(f"Total training agents: {args.num_envs * num_agents}")

    # Networks and training setup
    ...
    for env_idx in range(args.num_envs):
        ...
        for agent_idx in range(num_agents):
            ...
            if manager_key in obs_dict:
                ...
                # Find any manager key as fallback
                manager_keys = [k for k in obs_dict.keys() if k.startswith("manager_")]
                if manager_keys:
                    next_obs[global_agent_idx] = torch.tensor(obs_dict[manager_keys[0]], dtype=torch.float32)
                else:
                    # Ultimate fallback
                    next_obs[global_agent_idx] = torch.zeros(obs_dim, dtype=torch.float32)
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(num_steps):
            ...
            try:
                ...
                # Use previous observations and zero rewards on error
        ...
        if update % 10 == 0:
            logger.info(f"Update {update}/{num_updates}: avg_reward={avg_reward:.3f}, loss={avg_loss:.3f}")

        # Evaluation
        if update % algo_cfg.get("eval_frequency", 20) == 0:
            logger.info(f"--- Evaluation at update {update} ---")
            actor.eval()
            
            total_eval_reward = 0.0
            for ep in range(2):  # Quick evaluation
                ...
                while not done and steps < max_steps:
                    ...
                    try:
                        ...
                        logger.warning(f"Evaluation step error: {e}")
                        break
            ...
            logger.info(f"  Eval Return: {avg_eval_return:.3f}")
    ...
    logger.info(f"Training complete! Best eval return: {best_eval_return:.3f}")
